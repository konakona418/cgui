...
long long int cgui_application_messageCallback(CGUI_ComponentQuery query, UINT msg, WPARAM wParam, LPARAM lParam) {
    ...
    CGUI_Application* app = CGUI_APP_INSTANCE;
    CGUI_Result result = app->core->compManager->getComponentPredicate(app->core->compManager, &query.hwnd,
                                                                       cgui_application_predicateHwnd);
    if (is_ok(&result)) {
        CGUI_UIComponent* component = take(&result);
        query.component = component;
        // todo: this may lead to infinite recursive calls. proceed with caution.
        return component->eventHandler->handleEvent(component->eventHandler, query, msg, wParam, lParam,
                                                    cgui_application_messageCallback);
    } else {
